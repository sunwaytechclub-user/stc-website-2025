...
function createTextTexture(
  gl: GL,
  text: string,
  font: string = "30px bold sans-serif",
  color: string = "black",
  ...
  // Super sample the text for better quality
  const dpr = 4; // Increased from default devicePixelRatio for better quality
  context.font = font;
  ...
  // Make canvas much bigger for high resolution text
  ...
  context.lineWidth = 0.8; // Adjust for text thickness
  context.strokeText(text, (textWidth + 40) / 2, (textHeight + 40) / 2);
  ...
  return { 
    texture, 
    width: (textWidth + 40), // Return pre-scaled dimensions
...
class Title {
  ...
  constructor({
    ...
    text,
    textColor = "#545050",
    font = "20px sans-serif",  // Changed from DM Sans to sans-serif
    viewport,
  }: TitleProps) {
    autoBind(this);
    this.gl = gl;
  ...
  createMesh() {
    ...
    // Fix text scaling to be independent of image width
    const aspect = width / height;
    // Base the text size on viewport height instead of image scale
    const baseTextHeight = this.viewport.height * 0.015; // Try 0.025 for smaller text
    const textHeightScaled = baseTextHeight;
    const textWidthScaled = textHeightScaled * aspect;
    
    this.mesh.scale.set(textWidthScaled, textHeightScaled, 1);
    this.mesh.position.y = -this.plane.scale.y * 0.56;
...
class Media {
  ...
  createShader() {
    // Change texture settings to prevent blurring
  ...
  createTitle() {
    this.title = new Title({
      ...
      font: this.font,
      viewport: this.viewport  // Add this line
    });
  }
  ...
  onResize({
    ...
    // Modify these values to change image size
    const baseHeight = 800; // Change this value to adjust height
    const baseWidth = 1200;  // Change this value to adjust width
    
    this.scale = this.screen.height / 1500;
    this.plane.scale.y =
      (this.viewport.height * (baseHeight * this.scale)) / this.screen.height;
    this.plane.scale.x =
      (this.viewport.width * (baseWidth * this.scale)) / this.screen.width;
...
export default function CircularGallery({
  ...
  textColor = "#fff",
  borderRadius = 0.05,
  font = " 12px  sans-serif",  // Added 'bold' to make text heavier
